...
export default function ResumeRoaster() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      // 1. Check if the response is actually okay (status 200-299)
      if (!res.ok) {
      ...
      // 2. Double check if backend sent an error inside a 200 response
      if (data.error) {
      ...
      console.error("Fetch Error:", err); // Look at your Console tab for details
      setErrorType("network");
      // Show the actual error message if it exists, otherwise show generic network error
      setError(
              ...
              {!result && (
                ...
                >
                  ...
                  <p className={styles.disclaimer}>
                    Your resume is never stored. Analyzed and discarded
                    immediately.
                  </p>
                </motion.div>
              )}
